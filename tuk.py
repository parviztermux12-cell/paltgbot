...
# Множители для 12 карт - теперь более агрессивные, чтобы игра была сложнее
DECK_EFFECTS = {
    # Ход: (зеленый множитель, красный процент потерь, алмазный множитель)
    1:  (1.15, 0.30, 1.8),   # Ход 1 - красный стал опаснее
    2:  (1.2,  0.29, 2.0),   # Ход 2
    3:  (1.25, 0.25, 2.3),   # Ход 3 - красный теперь 25% потерь
    4:  (1.3,  0.32, 2.6),   # Ход 4
    5:  (1.35, 0.30, 3.0),   # Ход 5
    6:  (1.4,  0.32, 3.4),   # Ход 6
    7:  (1.45, 0.35, 3.8),   # Ход 7
    8:  (1.5,  0.38, 4.2),   # Ход 8
    9:  (1.55, 0.40, 4.7),   # Ход 9
    10: (1.6,  0.32, 5.2),   # Ход 10
    11: (1.7,  0.65, 5.8),   # Ход 11
    12: (2.0,  0.70, 7.0),   # Ход 12 - 50% потерь на красном
}
...
def deck_pick_card(call):
    try:
        ...
        with lock:
            ...
            # ИЗМЕНЕННЫЕ ШАНСЫ:
            # 45% зеленые
            # 45% красные (почти равны с зелеными)
            # 10% алмазы (редкий джекпот)
            rand = random.random()
            if rand < 0.45:
                card_type = "green"
                card_emoji = CARD_GREEN
            elif rand < 0.90:  # 0.45 + 0.45 = 0.90
                card_type = "red"
                card_emoji = CARD_RED
            else:  # 0.90 - 1.0 = 10%
                card_type = "diamond"
...
print("   • Шанс зеленых: 50% (было 65%)")
print("   • Шанс красных: 35% (было 20%)")
print("   • Шанс алмазов: 15%")
print("   • Красные карты теперь отнимают 20-50% от текущего выигрыша")
