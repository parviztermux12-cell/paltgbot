...
# ================== КОЛОДА СУДЬБЫ (СБАЛАНСИРОВАННАЯ ВЕРСИЯ) ==================
import uuid
import random
import threading
from telebot.types import InlineKeyboardMarkup, InlineKeyboardButton
...
# --- НАСТРОЙКИ ИГРЫ ---
MAX_BET = 10_000_000  # Максимальная ставка: 10 млн

# Множители для 12 карт - сделаны более щадящими
DECK_EFFECTS = {
    # Ход: (зеленый множитель, красный процент потерь, алмазный множитель)
    1:  (1.10, 0.15, 1.5),   # Ход 1
    2:  (1.15, 0.18, 1.8),   # Ход 2
    3:  (1.20, 0.20, 2.2),   # Ход 3
    4:  (1.25, 0.22, 2.6),   # Ход 4
    5:  (1.30, 0.25, 3.0),   # Ход 5
    6:  (1.35, 0.28, 3.5),   # Ход 6
    7:  (1.40, 0.30, 4.0),   # Ход 7
    8:  (1.45, 0.33, 4.6),   # Ход 8
    9:  (1.50, 0.36, 5.2),   # Ход 9
    10: (1.55, 0.39, 5.9),   # Ход 10
    11: (1.60, 0.42, 6.6),   # Ход 11
    12: (1.65, 0.45, 7.5),   # Ход 12
}
...
@bot.message_handler(func=lambda m: m.text and m.text.lower().startswith("колода "))
def start_deck_game(message):
    try:
        user_id = message.from_user.id
        parts = message.text.split()
        
        if len(parts) < 2:
            bot.reply_to(message, "❌ Укажи ставку\nПример: колода 5000")
            return
        ...
        try:
            ...
            if bet < 100:
                ...
                return
            # --- НОВОЕ: Проверка на максимальную ставку ---
            if bet > MAX_BET:
                bot.reply_to(message, f"❌ Максимальная ставка: {format_number(MAX_BAT)}$")
                return
        except ValueError:
...
def deck_pick_card(call):
    try:
        ...
        with lock:
            ...
            if not check_deck_owner(call, game["user_id"]):
                return
            ...
            # --- ИЗМЕНЕННЫЕ ШАНСЫ: Более сбалансированные ---
            # 40% зеленые, 40% красные, 20% алмазы
            rand = random.random()
            if rand < 0.40:
                card_type = "green"
                card_emoji = CARD_GREEN
            elif rand < 0.80:  # 0.40 + 0.40 = 0.80
                card_type = "red"
                card_emoji = CARD_RED
            else:  # 0.80 - 1.0 = 20%
                card_type = "diamond"
            ...
            step = min(game["step"], len(DECK_EFFECTS))
            green_mult, red_percent, diamond_mult = DECK_EFFECTS[step]
            
            current_win = int(game["bet"] * game["multiplier"])
            
            # Применяем эффект карты
            if card_type == "green":
                game["multiplier"] *= green_mult
                effect = f"🟢 +{int((green_mult-1)*100)}%"
            elif card_type == "red":
                # Красная карта отнимает процент от ТЕКУЩЕГО выигрыша
                damage = int(current_win * red_percent)
                # Гарантируем, что урон не может быть больше текущего выигрыша
                if damage >= current_win:
                    damage = current_win - 1
                new_win = max(1, current_win - damage)
            ...
            if game["step"] > MAX_STEPS or len(game["history"]) >= MAX_STEPS:
                ...
                # Выигрыш забирается автоматически, даже если меньше ставки.
                # Это честнее и проще для понимания.
                win_amount = current_win
                user_data = get_user_data(game["user_id"])
                user_data["balance"] += win_amount
                save_casino_data()
                
                result_text = f"🎉 <b>Ты открыл все карты!</b>"
        ...
        logger.error(f"Ошибка в deck_pick_card: {e}")
        bot.answer_callback_query(call.id, "❌ Ошибка!", show_alert=True)

@bot.callback_query_handler(func=lambda c: c.data.startswith("deck_cashout_"))
def deck_cashout(call):
    try:
        game_id = call.data.split("_")[2]
        ...
        with lock:
            ...
            # Забирает деньги, даже если выигрыш меньше ставки (чтобы было честно)
            user_data = get_user_data(game["user_id"])
            user_data["balance"] += win_amount
            save_casino_data()
            
            result_text = f"💰 <b>Ты забрал выигрыш!</b>"
            money_text = f"+{format_number(win_amount)}$"
...
print("   • Максимальная ставка: 10 млн$")
print("   • Шанс зеленых: 40%")
print("   • Шанс красных: 40%")
print("   • Шанс алмазов: 20%")
print("   • Множители и урон снижены для баланса")
