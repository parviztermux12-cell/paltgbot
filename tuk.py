...
def block_chat_access(message):
    ...
    bot.send_message(
        chat_id,
        f"{mention}\n\n<b>Ты убрал доступ к боту для всех пользователей в этом чате.</b>\n\n"
...
if 'config' not in globals():
    config = {
        "max_daily_income": 100000000,  # 100 млн в день
        "max_balance": 1000000000000000000000000000000,
        "transfer_daily_limit": 100000000,
        "transfer_fee": 0.1,
        "blackjack_win_multiplier": 2.0  # добавлен для блекджека
    }
...
def check_income_limits(user_id, amount):
    ...
    # Инициализируем daily_income если нет
    if "daily_income" not in user_data:
        user_data["daily_income"] = {"date": datetime.now().date().isoformat(), "amount": 0}
    
    # Сбрасываем счетчик если новый день
    if user_data["daily_income"]["date"] != datetime.now().date().isoformat():
        user_data["daily_income"] = {"date": datetime.now().date().isoformat(), "amount": 0}
        save_casino_data()
...
def add_income(user_id, amount, source="other"):
    ...
    if actual_amount > 0:
        ...
        # Обновляем daily_income
        if "daily_income" not in user_data:
            user_data["daily_income"] = {"date": datetime.now().date().isoformat(), "amount": actual_amount}
        else:
            user_data["daily_income"]["amount"] += actual_amount
            
        save_casino_data()
        logger.info(f"Начислен доход {actual_amount}$ пользователю {user_id} из источника {source}")
...
def apply_transfer_limits(sender_id, amount):
    ...
    # Инициализируем daily_transfers если нет
    if "daily_transfers" not in user_data:
        user_data["daily_transfers"] = {"date": datetime.now().date().isoformat(), "amount": 0}
    
    # Сбрасываем счетчик если новый день
    if user_data["daily_transfers"]["date"] != datetime.now().date().isoformat():
        user_data["daily_transfers"] = {"date": datetime.now().date().isoformat(), "amount": 0}
        save_casino_data()
