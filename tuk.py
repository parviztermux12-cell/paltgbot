...
def mines_handler(call):
    ...
    if action == "cancel" and not u["mines_started"]:  
        ...
        bot.edit_message_text(  
            ...
            call.message.message_id,  
            parse_mode="HTML",  
            reply_markup=mines_keyboard(user_id, reveal_all=True, hide_buttons=True)  # СКРЫВАЕМ КНОПКИ
        )  
        return  
    ...
    if action == "cash" and u["mines_started"]:  
        ...
        # НАЧИСЛЯЕМ ДЕНЬГИ - ПРАВИЛЬНО
        user_game_data = get_user_data(user_id)
        user_game_data["balance"] += win
        save_casino_data()  # Сохраняем изменения в основной базе
